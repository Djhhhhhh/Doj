...
import com.doj.dojbackendmodel.dto.question.JudgeConfig;
...
public class DefaultJudgeStrategy implements JudgeStrategy {
    ...
    public JudgeInfo doJudge(JudgeContext judgeContext) {
        ...
        for (int i = 0; i < judgeCaseList.size(); i++) {
            ...
            judgeCase.setOutput(judgeCase.getOutput().replace("\n", " "));
            judgeCase.setOutput(judgeCase.getOutput().trim());
            String actualOutput = outputList.get(i).trim();
            if (!judgeCase.getOutput().equals(actualOutput)) {
                judgeInfoMessageEnum = JudgeInfoMessageEnum.WRONG_ANSWER;
                judgeInfoResponse.setMessage(judgeInfoMessageEnum.getValue());
                return judgeInfoResponse;
            }
